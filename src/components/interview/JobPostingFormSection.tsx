    ...
    <div className="fixed inset-0 bg-black/13 flex items-center justify-center z-50">
      ...
      >
        <div className="relative mb-4.5">
          <Image src={character2} alt="캐릭터" width={158} height={154} className="object-contain" />
          <Image src={speechBubble} alt="말풍선" width={57} height={45} className="absolute -top-4 -right-6 object-contain" />
        </div>
        <div className="flex flex-col items-center gap-1.75 mb-12">
          <p className="h1 text-primary">질문을 생성 중이에요</p>
          <p className="p1 text-gray-2">공고에 특화된 질문을 AI가 생성하고 있습니다.</p>
        </div>
        <button
          type="button"
          onClick={onClose}
...
function CompletePopup({
  ...
  return (
    <div className="fixed inset-0 bg-black/13 flex items-center justify-center z-50">
      ...
      >
        <Image src={character3} alt="캐릭터" width={163} height={163} className="object-contain mb-5.5" />
        ...
        <Button className="w-70 rounded-full! py-3 cursor-pointer mt-12" onClick={onStart}>
...
function CalendarDropdown({
  ...
  return (
    <div ref={calendarRef} className="absolute bottom-18 right-0 z-50 bg-white rounded-xl shadow-[0_0_16px_0_rgba(99,99,99,0.16)] p-4 w-82">
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={onPrevMonth} className="cursor-pointer p-1 hover:text-primary">
          <IconChevronLeft size={20} className="text-gray-4 hover:text-text-primary" />
        </button>
        <span className="p4">{year}년 {month + 1}월</span>
        <button type="button" onClick={onNextMonth} className="cursor-pointer p-1 hover:text-primary">
          <IconChevronRight size={20} className="text-gray-4 hover:text-text-primary" />
        </button>
      </div>
      <div className="grid grid-cols-7 mb-2.5">
        {DAYS.map((day) => (
          <div key={day} className="text-center p4 text-gray-4">{day}</div>
        ))}
      </div>
      ...
      <div className="flex justify-end gap-6">
        <button type="button" onClick={onCancel} className="p4 text-gray-4 cursor-pointer hover:text-primary">취소</button>
        <button type="button" onClick={onConfirm} disabled={!tempDate} className={`p4 cursor-pointer ${tempDate ? 'text-primary' : 'text-gray-4'}`}>확인</button>
      </div>
    </div>
  )
}
...
export default function JobPostingFormSection() {
  ...
  const [popupState, setPopupState] = useState<'generating' | 'complete' | null>(null)

  const { ref: popupRef } = useModal(popupState !== null)
  const {
    calendarRef, selectedDate, dateInput, showCalendar,
    calendarYear, calendarMonth, tempDate, setTempDate,
    openCalendar, handleDateInputChange,
  ...
  return (
    <section className="flex flex-col flex-1 min-h-0 gap-5 px-10 pt-6.5 pb-10">
        ...
        <div className="flex flex-col gap-8.5 w-115">
          <div className="flex flex-col gap-5">
            <label className="p3 border-b border-gray-5 h-10">
              <span className="text-primary">채용 공고 링크</span>를 입력해주세요
            </label>
            <div className="relative">
              ...
              <div className="absolute left-0 top-full pt-1">
                <HelperText status={urlError ? 'error' : urlValid ? 'success' : 'empty'}>
                  {urlValid ? '링크가 확인되었습니다.' : '올바른 URL이 아닙니다. 다시 입력해주세요.'}
                </HelperText>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5 relative">
            ...
            <div className="relative">
              <InputBox
                ...
                rightElement={
                  <button type="button" onClick={openCalendar} className="cursor-pointer text-gray-4 hover:text-text-primary">
          ...
          <Button
            ...
            onClick={() => {
              ...
              // TODO: API 연동 시 제거
              setTimeout(() => setPopupState('complete'), 3000)
            }}
          >
      ...
      {popupState === 'generating' && (
        <GeneratingPopup popupRef={popupRef} onClose={() => setPopupState(null)} />
      )}
      {popupState === 'complete' && (
        <CompletePopup popupRef={popupRef} onStart={() => router.push('/interview/job-posting/countdown')} />
      )}
    </section>
  )
}
